...
void C_ConfigManager::setup() {
	...
	setup_item(&g_cfg.ragebot.auto_revolver, false, "Ragebot.auto_revolver");
...
{ false, "Head" },
	...
	{ false, "Foot" }
		...
		setup_item(&g_cfg.ragebot.baim_health[i], 50, start + ".baim_health");
		setup_item(&g_cfg.ragebot.baimaftershoots[i], 50, start + ".baim_shoots");
		setup_item(&g_cfg.ragebot.baim_key, BUTTON_CODE_NONE, "Ragebot.baim_key");
	...
	for (int i = 0; i < 3; i++) {
		...
		setup_item(&g_cfg.antiaim.DESYNCANTIAIM, BUTTON_CODE_NONE, "Ragebot.DESYNCANTIAIM");
	}
	...
	setup_item(&g_cfg.skins.AK47Skin, 0, "skins.AK47Skin");
	setup_item(&g_cfg.skins.P2000Skin, 0, "skins.P2000Skin");
	setup_item(&g_cfg.skins.RevolverSkin, 0, "skins.RevolverSkin");
	...
	setup_item(&g_cfg.misc.clantag, false, "Misc.clantag");
	...
	setup_item(&g_cfg.misc.fakewalk_speed, 0, "Misc.fakewalk_speed");
	setup_item(&g_cfg.misc.no_crouch_cooldown, false, "Misc.no_crouch_cooldown");
	setup_item(&g_cfg.misc.preversekillfeed, false, "Misc.preversekillfeed");

	// buybot config // All of the names and shit of the things in the multibox
	setup_item(&g_cfg.misc.buybot_extras, {
		{ false, "Kevlar" },
		{ false, "Kevlar&Helm" },
		{ false, "Defuser" },
		{ false, "Zeus" },
	...
	setup_item(&g_cfg.misc.buybot_rifleschoice, false, "Misc.riflechoice");
	setup_item(&g_cfg.misc.buybot_pistolchoice, false, "Misc.pistolchoice");
	...
	setup_item(&g_cfg.esp.knifeleft, false, "Esp.knifeleft");
	...
	setup_item(&g_cfg.esp.hands, false, "Esp.handsstyle");
	setup_item(&g_cfg.esp.hands_color, Color(255, 255, 255), "esp.hands_color");
	setup_item(&g_cfg.esp.hands_alpha, 0, "Esp.hands_alpha");
	...
	setup_item(&g_cfg.esp.croked_indicator, false, "Esp.croked_indicator");
	setup_item(&g_cfg.esp.extend_indicator, false, "Esp.extend_indicator");
	setup_item(&g_cfg.esp.lc_indicator, false, "Esp.lc_indicator");
	...
	setup_item(&g_cfg.esp.local_chams_fakelag, false, "Esp.local_chams_fakelag");
	setup_item(&g_cfg.esp.localfakelag_chams_color, Color(255, 255, 255), "Esp.localfakelag_chams_color");

	setup_item(&g_cfg.player.chams_opacitylocal1, 0, "Esp.local_chams_opacity");
	...
	setup_item(&g_cfg.esp.zeusrange_color, Color(255, 255, 255), "Esp.zeusrange_color");


	setup_item(&g_cfg.esp.grenadeprediction, false, "Esp.grenadeprediction");
	setup_item(&g_cfg.esp.grenadeprediction_color, Color(255, 255, 255), "Esp.grenadeprediction_color");
	setup_item(&g_cfg.esp.nightmode, false, "Esp.nightmode");
	setup_item(&g_cfg.esp.worldsupremacy, {
		{ false, "nightmode" },
		{ false, "fullbright" },
		}, "esp.worldsupremacy");


	setup_item(&g_cfg.esp.ColorSkyr, 0, "Esp.ColorSkyr");
	setup_item(&g_cfg.esp.ColorSkyg, 0, "Esp.ColorSkyg");
	setup_item(&g_cfg.esp.ColorSkyb, 0, "Esp.ColorSkyb");
	...
	setup_item(&g_cfg.esp.watermark, false, "Esp.watemark");
...
void C_ConfigManager::save(std::string config) {
	...
	auto get_dir = [&folder, &file, &config]() ->void {
		...
		if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_MYDOCUMENTS, NULL, 0, path))) {
			...
			file = std::string(path) + "\\animecheat.pw\\" + config;
		}
