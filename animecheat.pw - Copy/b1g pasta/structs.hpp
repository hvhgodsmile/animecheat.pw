...
#include "..\recode scatecheat3\sdk\interfaces\IClientEntity.hpp"
#include "..\recode scatecheat3\sdk\misc\EHandle.hpp"
#include "..\recode scatecheat3\sdk\misc\UtlVector.hpp"
#include "..\recode scatecheat3\sdk\math\QAngle.hpp"
#include "..\recode scatecheat3\utils\netmanager.hpp"
#include "..\recode scatecheat3\sdk\misc\threadtools.h"
#include "..\recode scatecheat3\sdk\misc\CBoneAccessor.hpp"
#include "..\recode scatecheat3\sdk\misc\CBoneAccessor.hpp"
...
public:
	float m_flLifetime; //0x004C 
	float m_flLocalPlayerLifetimeMod; //0x0050 
	float m_flFadeOutTime; //0x0054 
private:
	char pad_0x0058[0x3E8]; //0x0058
public:
	void clear_notices();
};
...
{
	static auto pThis = *reinterpret_cast<DWORD**>(util::pattern_scan("client_panorama.dll", "B9 ? ? ? ? E8 ? ? ? ? 85 C0 0F 84 ? ? ? ? 8D 58") + 1);
...
public:
	...
	{
		return static_cast<entity_t*>(g_csgo.m_entitylist()->GetClientEntity(index));
	}
...
public:
	...
	NETVAR(short, m_iItemDefinitionIndex1, "CBaseAttributableItem", "m_iItemDefinitionIndex");
...
public:
	...
	{
		return static_cast<player_t*>(GetEntityByIndex(g_csgo.m_engine()->GetPlayerForUserID(id)));
	}
	static __forceinline player_t* GetPlayerByIndex(int i)
	{
		return static_cast<player_t*>(GetEntityByIndex(i));
	}
	static __forceinline player_t *GetLocalPlayer(const bool set = false)
	{
		static player_t *local = nullptr;

		if (!local || set)
			local = GetPlayerByIndex(g_csgo.m_engine()->GetLocalPlayer());
	...
	NETVAR(bool, m_ragPos, "CRagdoll", "m_ragPos");
	...
	NETVAR(float, m_flPoseParameter1, "DT_BaseAnimating", "m_flPoseParameter");
	...
	NETVAR(float, m_flLowerBodyYawTarget1, "CCSPlayer", "m_flLowerBodyYawTarget");
	...
	float& get_old_simulation_time(void) {
		static auto offset = netvars::get().get_offset("CBaseEntity", "m_flSimulationTime");
		return *(float*)((uintptr_t)this + offset + 0x4);
	}

	Vector get_ragdoll_pos()
	{
		return *reinterpret_cast<Vector*>(uintptr_t(this) + m_ragPos());
	}
	...
	float get_max_desync_delta(player_t* local);
	...
	int get_life_state() { //m_lifeState                                        
