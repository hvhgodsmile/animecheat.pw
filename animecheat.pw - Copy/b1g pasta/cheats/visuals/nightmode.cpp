...
void nightmode::modulate(MaterialHandle_t i, IMaterial *material, bool backup = false) {
	...
	float sky_r = g_cfg.esp.ColorSkyr / 10;
	float sky_g = g_cfg.esp.ColorSkyg / 10;
	float sky_b = g_cfg.esp.ColorSkyb / 10;

	if (strstr(material->GetTextureGroupName(), "World")) {
		if (backup) materials.push_back(MaterialBackup(i, material));

		material->ColorModulate(0.11f, 0.11f, 0.15f);
		material->SetMaterialVarFlag(MaterialVarFlags_t::MATERIAL_VAR_TRANSLUCENT, false);
	...
	{
		material->ColorModulate(sky_r, sky_g, sky_b);

	}
}
// pcode supremacy dumo shit
void nightmode::fullbright1apply() {
...
void nightmode::fullbright1remove() {
	if (!executed || !g_csgo.m_cvarspoofer()->ready()) {
		return;
	}

	executed = false;
	g_cfg.esp.worldsupremacy[fullbright1].enabled = false;
...
void nightmode::remove() {
	...
	executed = false;
	g_cfg.esp.worldsupremacy[nightmode1].enabled = false;
