...
void playeresp::draw_weapon(player_t* m_entity, Box box) {
	...
	render::get().text(fonts[ESPNAME], box.x + (box.w / 2), _y, color, HFONT_CENTERED_X, weapon->get_name());
}
...
void playeresp::draw_flags(player_t * e, Box box) {
	...
	//			render::get().text(fonts[ESPFLAG], box.x + (box.w / 2), box.y - 22, Color::Red, HFONT_CENTERED_X, "resolver off.");
	//	}
	//}
...
void playeresp::pov_arrow()
{
	...
	auto draw_arrow = [&](float rotation, Color color) -> void {
		render::get().triangle(
			Vector2D(screen_center_x + cosf(rotation) * radius, screen_center_y + sinf(rotation) * radius),
			Vector2D(screen_center_x + cosf(rotation + DEG2RAD(2)) * (radius - 16),
				screen_center_y + sinf(rotation + DEG2RAD(2)) * (radius - 16)),
			Vector2D(screen_center_x + cosf(rotation - DEG2RAD(2)) * (radius - 16),
				screen_center_y + sinf(rotation - DEG2RAD(2)) * (radius - 16)),
			color
		);
	...
	for (auto i = 1; i <= g_csgo.m_globals()->m_maxclients; i++) {
		...
		if (screen_point.x < 0 || screen_point.y < 0 || screen_point.x > screen_width || screen_point.y > screen_height) {
			...
			const auto angle_yaw_rad = DEG2RAD(client_viewangles.y - angle.y - 90);

			Color color = g_cfg.player.out_pov_arrow_color;
			color.SetAlpha(alpha);

			draw_arrow(angle_yaw_rad, color);
		}
	}
}
