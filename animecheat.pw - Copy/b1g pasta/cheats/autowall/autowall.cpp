...
bool autowall::classname_is(IClientEntity* e, const char* class_name) {
	return !strcmp(e->get_classname(), class_name);
}
bool autowall::is_breakable_entity(IClientEntity* e) {
	for (int i = 1; i <= g_csgo.m_globals()->m_maxclients; i++) {
		...
		static auto __rtdynamiccast_fn = util::pattern_scan("client_panorama.dll", "6A 18 68 ? ? ? ? E8 ? ? ? ? 8B");
		static auto is_breakable_entity_fn = util::pattern_scan("client_panorama.dll", "55 8B EC 51 56 8B F1 85 F6 74 68 83 BE");
		...
		int(__thiscall ***v4)(IClientEntity*);
		int v5;

		if (e && (e->m_iHealth() >= 0 || (*((int(**)(void))*(IClientEntity**)e + 122))() <= 0) && *((uint8_t*)e + 636) == 2) {
			void* v3 = ((IClientEntity**)e)[284];

			if (v3 != (void *)17 && v3 != (void *)6 && v3)
				return false;
			...
			if (!classname_is(e, "func_breakable") && !classname_is(e, "func_breakable_surf")) {
				if ((*((int(__thiscall**)(IClientEntity*))*(IClientEntity**)e + 148))(e) & 0x10000)
					return false;

				goto label_18;
			}

			if (!classname_is(e, "func_breakable_surf") || !*((uint8_t*)e + 2564)) {
			label_18:
				__asm {
					push 0
