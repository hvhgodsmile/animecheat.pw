...
void engineprediction::start() {
	...
	if (!m_random_seed || !m_prediction_player) {
		m_random_seed = *reinterpret_cast<int **>(util::pattern_scan("client_panorama.dll", "8B 0D ? ? ? ? BA ? ? ? ? E8 ? ? ? ? 83 C4 04") + 2);
		m_prediction_player = *reinterpret_cast<int **>(util::pattern_scan("client_panorama.dll", "89 35 ? ? ? ? F3 0F 10 46") + 2);
	}
