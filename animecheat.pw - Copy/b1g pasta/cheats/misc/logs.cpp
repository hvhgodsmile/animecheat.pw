...
void eventlogs::paint_traverse() {
	...
	auto last_y = 6;
	const auto font = fonts[SUBLIMINALTEXT];
	const auto name_font = fonts[SUBLIMINALBOLD];
	auto name_size = render::get().text_width(name_font, "[animecheat.pw]");
	...
	for (unsigned int i = 0; i < logs.size(); i++) {
		...
		auto front_color = Color::Skeet;
		front_color.SetAlpha(log.color.a());

		render::get().text(name_font, log.x, last_y + log.y, Color::Skeet, HFONT_CENTERED_NONE, "[animecheat.pw]");
