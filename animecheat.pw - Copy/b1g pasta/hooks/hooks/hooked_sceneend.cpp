...
void __fastcall hooks::hooked_sceneend( void * thisptr, void * edx ) {
	...
	if ( g_cfg.esp.ghost_chams && g_ctx.m_local->is_alive( ) ) {
		...
		doggy->ColorModulate(g_cfg.esp.ghost_chams_color[0], g_cfg.esp.ghost_chams_color[1], g_cfg.esp.ghost_chams_color[2]);
...
#define clamp(val, min, max) (((val) > (max)) ? (max) : (((val) < (min)) ? (min) : (val)))
	...
	if (g_ctx.m_local->m_vecVelocity().Length2D() > 30 && g_ctx.m_local && g_cfg.esp.local_chams_fakelag && g_ctx.m_local->is_alive())
	{
		...
		float clr[4] = {
			g_cfg.esp.localfakelag_chams_color[0] / 255.f,
			g_cfg.esp.localfakelag_chams_color[1] / 255.f,
			g_cfg.esp.localfakelag_chams_color[2] / 255.f,
			g_cfg.esp.localfakelag_chams_color[3] / 255.f
		};

		g_csgo.m_renderview()->SetColorModulation(clr);
		doggy->ColorModulate(g_cfg.esp.localfakelag_chams_color[0], g_cfg.esp.localfakelag_chams_color[1], g_cfg.esp.localfakelag_chams_color[3]);
