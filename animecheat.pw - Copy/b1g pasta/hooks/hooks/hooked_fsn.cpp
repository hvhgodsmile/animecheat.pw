...
void __stdcall hooks::hooked_fsn(ClientFrameStage_t stage) {
	...
	if (stage == FRAME_RENDER_START) {
		...
		for (int i = 1; i <= g_csgo.m_entitylist()->GetHighestEntityIndex(); i++) {
			...
			if (e->GetClientClass()->m_ClassID == ClassId_CCSRagdoll) {
				...
				if (g_cfg.misc.exaggerated_ragdolls) {
					vec3_t
						new_force = ragdoll->m_vecForce() * -999,
						new_velocity = ragdoll->m_vecRagdollVelocity() * -999;
		...
		if (g_cfg.misc.preversekillfeed && g_ctx.m_local->is_alive())
			otheresp::get().PreserveKillfeed();
	...
	if (stage == FRAME_RENDER_END) {
		if (g_cfg.esp.worldsupremacy[nightmode1].enabled)
			nightmode::get().apply();
		else
			nightmode::get().remove();


		if(g_cfg.esp.worldsupremacy[fullbright1].enabled)
			nightmode::get().fullbright1apply();
